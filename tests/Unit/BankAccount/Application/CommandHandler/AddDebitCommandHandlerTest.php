...
use App\BankAccount\Application\CommandHandler\AddDebitCommandHandler;
...
class AddDebitCommandHandlerTest extends TestCase
{
    public function testInvoke(): void
    {
        ...
        $commandHandler = new AddDebitCommandHandler($eventBus, $accountRepository);

        $uuidService = new UuidService();

        $command = new AddDebitCommand(
        ...
        $account->expects($this->once())
            ...
            ->with(
                $this->callback(function (Transaction $transaction) use ($command) {
                    ...
                    $this->assertEquals(Type::DEBIT, $transaction->getType());
        ...
        $account->expects($this->once())
            ->method('pullEvents')
            ->willReturn([$this->createMock(DebitTransactionAddedEvent::class)]);
        ...
        $eventBus->expects($this->exactly(1))
            ->method('dispatch')
            ->with($this->isInstanceOf(DebitTransactionAddedEvent::class));
